...
def getCategories(catType, language="en", resume_from=None):
    ...
    Parameters:
      ...
      - language: "en" (default) or "de". If "de", appends the German language parameter to requests.
      - resume_from: an item ID from which to resume processing.
    
    Any failed API calls are recorded in an error list and saved to errors_<catType>.json.
    """
    base_url = "https://api.guildwars2.com/v2/"
    list_url = base_url + catType
    # Append language query for the list of IDs if German is selected
    if language == "de":
        list_url += "?lang=de"
    ...
    if resume_from is not None:
        try:
            ...
            list_of_ids = list_of_ids[start_index:]
        except ValueError:
            print(f"ID {resume_from} not found in the list. Starting from the beginning.")
    
    items_data = []
    error_list = []
    batch_size = 200  # Maximum number of IDs per request
    
    for i in range(0, len(list_of_ids), batch_size):
        batch_ids = list_of_ids[i:i+batch_size]
        ids_str = ",".join(str(x) for x in batch_ids)
        # Build URL for batch request. Note: for German, use '&lang=de' because the first query parameter is already used.
        item_url = f"{base_url}{catType}?ids={ids_str}"
        if language == "de":
            item_url += "&lang=de"
        ...
        try:
            ...
            if item_resp.status_code in [429, 502]:
                print(f"Rate limit or temporary error (status {item_resp.status_code}) for batch {batch_ids}. Waiting 5 seconds and retrying...")
                time.sleep(5)
                item_resp = requests.get(item_url)
            ...
            for item in items:
                # Skip item if data is incomplete (missing 'name' or 'id')
            ...
            print(f"Processed batch starting with ID {batch_ids[0]}")
...
def fetch_single_item(item_id, cat_type, retry_delay=5):
    """
    Tries to fetch a single item repeatedly until a successful response is received.
    If the item returns incomplete data, it is skipped.
    """
    base_url = "https://api.guildwars2.com/v2/"
    while True:
        try:
            ...
            if response.status_code in (429, 502):
                print(f"Item {item_id}: Too many requests (status {response.status_code}). Waiting {retry_delay} seconds...")
                time.sleep(retry_delay)
                continue
            response.raise_for_status()
            item_json = response.json()
            # Check if the object has both a name and an id. If not, skip it.
            if item_json.get("name") and item_json.get("id"):
                ...
                print(f"Item {item_id}: Incomplete data. Skipping item.")
                return None
        except requests.exceptions.RequestException as e:
            print(f"Item {item_id}: Exception {e}. Waiting {retry_delay} seconds before retrying...")
            time.sleep(retry_delay)


def retry_failed_items(cat_type, error_file="errors_items.json", output_file="items_retry_success.json"):
    """
    Reads the error file, retries each failed API request, and saves successfully retrieved items.
    """
    try:
        with open(error_file, "r") as f:
            error_items = json.load(f)
    except Exception as e:
        print(f"Error loading error file: {e}")
        return
    ...
    for error_obj in error_items:
        ...
        print(f"Retrying item {item_id}...")
        result = fetch_single_item(item_id, cat_type)
        if result:
            successful_results.append(result)
            print(f"Item {item_id} successfully retrieved.")
        else:
            print(f"Item {item_id} could not be retrieved after multiple attempts.")

    try:
        with open(output_file, "w") as f:
            json.dump(successful_results, f, indent=2)
        print(f"Successfully retried items have been saved in '{output_file}'.")
    except Exception as e:
